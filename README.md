...
1. アプリの公開ディレクトリを `/home/u685478147/public_html/public_html/notion_ledger_sync` に配置します。
2. Laravel の非公開ファイル（`app`、`bootstrap`、`config`、`database`、`resources`、`routes`、`storage`、`vendor` など）は Web から直接見えない場所に置くか、サーバー設定で `notion_ledger_sync` の DocumentRoot をこのプロジェクトの `public` ディレクトリに向けます。
3. 共有側の `/home/u685478147/public_html/public_html/.htaccess` には、このアプリ用の rewrite を追加しません。
4. `.env` は以下のように設定します。
